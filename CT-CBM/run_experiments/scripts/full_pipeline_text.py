...
from models.jointCBM_text import JointModel
...
class JointResidualFittingModel(nn.Module):
    def __init__(self, joint_model, linear_layer, discovery_tokenizer, config):
    ...
    def compute_connection_importance(self, train_loader):
        ...
        for train_batch in train_loader:
            ...
            train_masks_ = train_batch["attention_mask"].to(self.device)

            # Residual part
            with torch.no_grad():
                pooled_output = self.get_pooled_output(train_inputs_, train_masks_)
