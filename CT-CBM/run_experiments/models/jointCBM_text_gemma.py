...
class JointModel:
    def __init__(self, model, tokenizer, ModelXtoCtoY_layer, config, train_loader, val_loader, num_epochs=1, use_cls_token = True, num_discoverded_concepts_ = 1, linear_layer = None):
        ...
        # self.optimizer = Adam(
        #     list(self.ModelXtoCtoY_layer.parameters()),
        #     lr=1e-5)
    ...
    # NEW
    def forward_linear(self, input_ids):
        pooled_output = self.get_pooled_output(input_ids)
    ...
    # used in LIG 
    def forward_residual_layer(self, input_ids):
    ...
    def forward_per_concept(self, input_ids, concept_number = 1):
    ...
    def train_model(self, train_loader, val_loader):
        ...
        for epoch in range(self.num_epochs):
            ...
            val_metrics = self.evaluate_model(self.val_loader)
    ...
    def train_residual_layer(self, train_loader, num_epochs):
        ...
        for epoch in range(num_epochs):
            ...
            for batch in tqdm(train_loader, desc="Train", unit="batch"):
                ...
                final_output, XtoC_output, pooled_output = self.forward_linear(input_ids)
    ...
    def evaluate_model_with_residual(self, data_loader, dataset_name =".."):
        ...
        with torch.no_grad():
            for batch in tqdm(data_loader, desc= dataset_name, unit="batch"):
                ...
                final_output, XtoC_output, _ = self.forward_linear(input_ids)
    ...
    def load_model(self):
        if self.alternate_save:
            ...
            self.embedder_model.load_state_dict(torch.load(f"{self.config.SAVE_PATH}blue_checkpoints/{self.model_name}/BaselineModel/{self.model_name}_state_dict.pth"))
