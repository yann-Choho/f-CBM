...
PATH_DATA_dbpedia = PATH+"/datasets/dbpedia"
...
class dbpediaDataset(Dataset):
    def __init__(self, train_val_test='train', dataset_type='C3M', select_concepts=None, random_concepts=None):
        ...
        if(train_val_test=='train'):
            self.dataset = pd.read_csv(os.path.join(PATH_DATA_dbpedia, f'df_with_topics_v4_{dataset_type.replace("CBLLM","CB_LLM")}.csv'))
        else:
            self.dataset = pd.read_csv(os.path.join(PATH_DATA_dbpedia, f'df_with_topics_v4_{dataset_type.replace("CBLLM","CB_LLM")}_test.csv'))
        ...
        # convert concept columns to floats
        for concept in self.concept_list:
            #print(concept)
            #print(self.dataset[concept].value_counts())
            self.dataset[concept] = self.dataset[concept].replace('partially','0').replace('protection efforts','1').astype(float)
            #print(self.dataset[concept].value_counts())
            #print('')
        ...
        self.class_dict = {0: 'Company', 1: 'Artist', 2: 'Athlete', 3: 'Building', 4: 'Animal', 5: 'Film'}
...
def load_data_dbpedia(batch_size=32, num_workers=4, dataset_type='C3M', combine_type='text', select_concepts=None, random_concepts=None):

    if(combine_type != 'text'):
        raise ValueError("No combine_type other than text is supported")

    # Create subsets
    train_dataset = dbpediaDataset(train_val_test='train', dataset_type=dataset_type, select_concepts=select_concepts, random_concepts=random_concepts)

    # concept_list
    concept_list = train_dataset.concept_list

    # select same concepts as train dataset
    val_dataset = dbpediaDataset(train_val_test='val', dataset_type=dataset_type, select_concepts=[concept.replace('concept_text_', '') for concept in concept_list])
    test_dataset = dbpediaDataset(train_val_test='test', dataset_type=dataset_type, select_concepts=[concept.replace('concept_text_', '') for concept in concept_list])
