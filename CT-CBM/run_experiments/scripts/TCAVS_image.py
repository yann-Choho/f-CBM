...
class TCAV():
    ...
    def __init__(self, concepts, baseline_model, embedder_tokenizer=None, batch_size=64, verbose=False, svm_params_dict=None, config=None, train_loader=None, val_loader=None, test_loader=None):
        ...
        self.embedder_tokenizer = embedder_tokenizer  # Pas utilisé en image_only
        self.seed = 42
    ...
    def get_embeddings_and_labels(self, dataloader, concept, layer_num=-1, use_cls_token=True):
        """
        Extrait les embeddings et labels pour un concept donné depuis un DataLoader d'images.
        
        Args:
            dataloader: DataLoader contenant des batches avec 'image' et concepts
            concept: Nom du concept à extraire
            layer_num: Couche à utiliser (non utilisé ici, pour compatibilité)
            use_cls_token: Token CLS (non utilisé en image, pour compatibilité)
            
        Returns:
            embeddings: np.array des embeddings extraits
            labels: np.array des labels du concept
        """
        all_embeddings = []
        all_labels = []
        ...
        for batch in dataloader:    
            ...
            # Extraction des embeddings via le modèle vision
            with torch.no_grad():
                # Méthode 1: get_pooled_output (CLIP/BLIP)
                if hasattr(self.baseline_model, 'get_pooled_output'):
                    pooled_output = self.baseline_model.get_pooled_output(pixel_values=images)
                # Méthode 2: get_image_features (CLIP)
                elif hasattr(self.baseline_model, 'get_image_features'):
                    pooled_output = self.baseline_model.get_image_features(pixel_values=images)
                # Méthode 3: vision_model direct
                elif hasattr(self.baseline_model, 'vision_model'):
                    pooled_output = self.baseline_model.vision_model(pixel_values=images).pooler_output
                else:
    ...
    def undersample_dataloader(dataloader, concept, balanced=True, closest=False):
        ...
        for class_label, count in class_counts.items():
            ...
            if count == min_class_count:
                ...
                if closest:
                    # Pour image_only, on utilise les embeddings image pour calculer les distances
                    # Note: Cela nécessiterait d'extraire les embeddings, ce qui est coûteux
                    # Pour simplifier, on désactive cette option en image_only
                    print("⚠️ Option 'closest' non supportée en image_only, utilisation de l'échantillonnage aléatoire")
    ...
    def learn_cav(self, dataloader, concept, layer_num=-1, linear_classifier=True, 
        ...
        if linear_classifier or logistic_classifier:
            ...
            cav = np.zeros(512)  # ✅ Dimension image pour CLIP/BLIP
    ...
    def get_gradients_per_class(self, pixel_values, class_idx=0):
        """
        Calcule le gradient par rapport à `pooled_output` pour une classe donnée (image_only).

        Args:
        - pixel_values: Les valeurs de pixels de l'image.
        - class_idx: L'indice de la classe pour laquelle calculer le gradient.

        Returns:
        - gradients: Les gradients calculés par rapport à `pooled_output`.
        """
        ...
        # Extraction des embeddings avec gradient activé
        if hasattr(self.baseline_model, 'get_pooled_output'):
            pooled_output = self.baseline_model.get_pooled_output(pixel_values=pixel_values)
        elif hasattr(self.baseline_model, 'get_image_features'):
            pooled_output = self.baseline_model.get_image_features(pixel_values=pixel_values)
        elif hasattr(self.baseline_model, 'vision_model'):
            pooled_output = self.baseline_model.vision_model(pixel_values=pixel_values).pooler_output
        else:
            raise ValueError("Méthode d'extraction embeddings image non reconnue")
    ...
    def partition_dataloader_by_labels(self, main_dataloader):
        ...
        for idx, batch in enumerate(main_dataloader):
            ...
            for i, label in enumerate(labels):
                ...
                if label_item not in indices_by_label:
                    indices_by_label[label_item] = []
                indices_by_label[label_item].append(idx * main_dataloader.batch_size + i)
    ...
    def calculate_tcav_scores_non_linearity_hypothesis_20(self, dataloader=None, max_examples_per_concept=20):
        ...
        de 20 exemples par concept (indépendamment des classes) - VERSION IMAGE_ONLY.
        ...
        for concept in self.concepts:
            ...
            for class_idx in range(num_classes):
                ...
                for idx, example in enumerate(examples):
                    images = example['image'].unsqueeze(0).to(self.device)
                                            
                    # Obtenir les gradients pour cette classe
                    grads = self.get_gradients_per_class(images, class_idx=class_idx)
    ...
    def calculate_tcav_scores(self, dataloader=None, use_subset=True):
        ...
        for class_idx in tqdm(range(num_classes), desc="Classes", unit='class'):
            ...
            images = batch['image'][0].unsqueeze(0).to(self.device)
