...
from models.jointCBM_multimodal import JointModel
...
class JointResidualFittingModel(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, pixel_values):
        return self.joint_model.forward_residual(input_ids, attention_mask, pixel_values)

    # OK
    def get_pooled_output(self, input_ids, attention_mask, pixel_values):
        """ get the embedding of the input text"""
        return self.joint_model.get_pooled_output(input_ids, attention_mask, pixel_values)
    ...
    def compute_connection_importance(self, train_loader):
        ...
        for train_batch in train_loader:
            ...
            images = train_batch['image'].to(self.device)
            texts = train_batch['text']#.to(self.device)
            
            # self.tokenizer
            inputs = self.joint_model.tokenizer(text=texts, images=images, return_tensors="pt", padding=True, truncation=True).to(self.device)
    
            # Residual part
            with torch.no_grad():
                pooled_output = self.joint_model.get_pooled_output(**inputs)
        ...
        for iteration in range(num_iterations):
            ...
            if iteration == 0:
                ...
                if len(self.joint_model.concepts_name) == 0:
                    ...
                    CBM_joint = JointModel(backbone=self.config.model_name, 
                       ModelXtoCtoY_layer = ModelXtoCtoY_layer, 
                       config = self.config, 
