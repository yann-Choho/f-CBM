...
class CUBDataset(Dataset):
    def __init__(self, root, transform=None, dataset_type='C3M', combine_type='combine', select_concepts=None, random_concepts=None):
        self.root = root
        ...
        # import dataset file
        if(dataset_type == 'C3M'):
            self.dataset = pd.read_csv(os.path.join(root, 'cub_concept_dataset_new_with_text.csv'))
            if(self.combine_type =='concat'):
                raise ValueError("No separate text/image concepts for CUB C3M, use combine mode instead")
            if(self.combine_type=='text'):
                raise ValueError("No text concepts for CUB C3M")
        elif(dataset_type == 'CBLLM'):
            self.dataset = pd.read_csv(os.path.join(root, 'cub_concept_dataset_new_with_text_cbllm.csv'))
        ...
        # same order for concept columns
        if(self.dataset_type == 'CBLLM'):
            concept_cols = np.unique([column.replace('concept_text_', 'concept_') for column in self.dataset.columns if column.startswith('concept_text_')] + [column.replace('concept_image_', 'concept_') for column in self.dataset.columns if column.startswith('concept_image_')])
            # identify non concept columns
            non_concept_cols = [column for column in self.dataset.columns if not column.startswith('concept_')]
            self.dataset = self.dataset[non_concept_cols + [cols.replace('concept_', suffix) for cols in concept_cols for suffix in ['concept_text_','concept_image_']]]
        else:
            concept_cols = [column for column in self.dataset.columns if column.startswith('concept_')]
            non_concept_cols = [column for column in self.dataset.columns if not column.startswith('concept_')]
            self.dataset = self.dataset[non_concept_cols + concept_cols]

        # combine for cbllm dataset
        if((self.combine_type == 'combine') and (self.dataset_type == 'CBLLM')):

            for col in concept_cols:
                # sum of activations
                self.dataset[col] = self.dataset[col.replace('concept_', 'concept_text_')] + self.dataset[col.replace('concept_', 'concept_image_')]

            if(self.dataset_type != 'CBLLM'):
                self.dataset[concept_cols] = self.dataset[concept_cols].clip(upper=1)

            # drop columns starting with concept_text_
            self.dataset = self.dataset.drop([column for column in self.dataset.columns if column.startswith('concept_text_')], axis=1)
            # same for image
            self.dataset = self.dataset.drop([column for column in self.dataset.columns if column.startswith('concept_image_')], axis=1)

            self.dataset = self.dataset.reset_index(drop=True)

        # combine_type = text for CBLLM
        if((self.combine_type == 'text') and (self.dataset_type == 'CBLLM')):
            self.dataset = self.dataset.drop([column for column in self.dataset.columns if column.startswith('concept_image_')], axis=1)

        # combine_type = image for CBLLM
        if((self.combine_type == 'image') and (self.dataset_type == 'CBLLM')):
            self.dataset = self.dataset.drop([column for column in self.dataset.columns if column.startswith('concept_text_')], axis=1)

        # combine_type = image for C3M
        if((self.combine_type == 'image') and (self.dataset_type == 'C3M')):
            pass
    ...
    def __getitem__(self, idx):
        ...
        return {'image_id': image_id , 'text':text,"image": image, "label": label, **concept_dict}
...
def logreg(data, class_dict):
    ...
    X = data.drop(columns=['class_label', 'class_id', 'path', 'image_id', 'target', 'split', 'text'])
    y = data['target']
...
def load_data_CUB(data_dir, batch_size=32, num_workers=4, dataset_type='C3M', combine_type='combine', select_concepts=None, random_concepts=None):
    ...
    full_dataset = CUBDataset(root=data_dir, transform=transform, dataset_type=dataset_type, combine_type=combine_type, select_concepts=select_concepts, random_concepts=random_concepts)
