...
import time
import torch
import torch.nn.functional as F  # ADDED: Import needed for normalize function
import numpy as np
...
class JointModel:
    def __init__(self, model, tokenizer, ModelXtoCtoY_layer, config, train_loader, val_loader, num_epochs=2, use_cls_token = True, num_discoverded_concepts_ = 1, linear_layer = None):
        ...
        # MODIFIED: Keep BCE as backup, add cosine cubed loss option
        self.loss_concept_function = BCEWithLogitsLoss()
        self.use_cosine_cubed_loss = getattr(config, 'use_cosine_cubed_loss', True)  # Default to new loss
    ...
    # NEW: Cosine cubed similarity loss function inspired by CB_LLM
    def cos_sim_cubed(self, cbl_features, target):
        """
        Cosine cubed similarity loss function from CB_LLM source code.
        
        Args:
            cbl_features: Concept logits/features from the model
            target: Target concept labels
        
        Returns:
            Mean cosine cubed similarity
        """
        # Center the features by subtracting mean
        cbl_features = cbl_features - torch.mean(cbl_features, dim=-1, keepdim=True)
        target = target - torch.mean(target, dim=-1, keepdim=True)
        
        # Normalize cubed features
    ...
    def train_model(self, train_loader, val_loader):
        ...
        for epoch in range(self.num_epochs):
            ...
            for batch in tqdm(train_loader, desc="Training", unit="batch"):
                ...
                # MODIFIED: Use cosine cubed loss or BCE based on configuration
                if self.use_cosine_cubed_loss:
                    # New cosine cubed similarity loss
                    XtoC_loss = -self.cos_sim_cubed(XtoC_logits, concept_labels)  # Negative because we want to maximize similarity
                else:
                    # Original BCE loss (sigmoid already integrated in loss function for numerical stability)
                ...
                # MODIFIED: Accuracy et F1 par concept (adapt to new loss function)
                for i, concept in enumerate(self.concepts_name):
                    if self.use_cosine_cubed_loss:
                        # For cosine cubed loss, we need to apply sigmoid manually for evaluation
                        concept_pred = torch.round(torch.sigmoid(XtoC_output[i])).cpu().detach().numpy()
                    else:
                        # For BCE loss, sigmoid is already handled in the loss function
                        concept_pred = torch.round(torch.sigmoid(XtoC_output[i])).cpu().detach().numpy()
            ...
            # ADDED: Log which loss function is being used
            loss_type = "Cosine Cubed" if self.use_cosine_cubed_loss else "BCE"
            print(f"Epoch {epoch + 1}: Using {loss_type} loss for concepts")
    ...
    # evaluate_model method remains unchanged as it uses sigmoid for evaluation regardless of loss function
