...
class JointModel:
    def __init__(
        self,
        backbone: str,  #was before model and tokenizer
        ModelXtoCtoY_layer,
        config,
        train_loader,
        val_loader,
        ...
        # NEW 
        # Initialize encoder and processor based on chosen backbone
        if backbone == 'clip':
        ...
        if self.linear_layer is not None: # ADDED JUST FOR AVOIDING ERROR
            self.linear_layer.to(self.device) # ADDED JUST FOR AVOIDING ERROR
    ...
    def get_pooled_output(self, input_ids, attention_mask, pixel_values):
        ...
        if(not self.multimodal_features):
            outputs = self.embedder_model(input_ids=input_ids, attention_mask=attention_mask, pixel_values=pixel_values)
            image_features = outputs.image_embeds
            text_features = outputs.text_embeds
            combined_features = torch.cat((text_features, image_features), dim=1)
        else:
            combined_features = self.embedder_model.get_multimodal_features(input_ids=input_ids, pixel_values=pixel_values,attention_mask=attention_mask)
    ...
    def forward(self, batch):
        """
        Forward pass: encode image+text, then predict concepts and task label.
        """
        ...
        self.tokenizer
        inputs = self.tokenizer(text=texts, images=images, return_tensors="pt", padding=True, truncation=True).to(self.device)

        pooled = self.get_pooled_output(**inputs)
        outputs = self.ModelXtoCtoY_layer(pooled)
        # outputs: [task_logits, concept_logits...] or similar structure
        return outputs, labels

    # NEW
    def forward_linear(self, input_ids, attention_mask, pixel_values):
    ...
    # MODIFIED
    # used in LIG 
    def forward_residual_layer(self, input_ids, attention_mask, pixel_values):
    ...
    def forward_per_concept(self, input_ids, attention_mask, pixel_values, concept_number = 1):
    ...
    def train_residual_layer(self, train_loader, num_epochs):
        ...
        for epoch in range(num_epochs):
            ...
            for batch in tqdm(train_loader, desc="Train", unit="batch"):
                ...
                final_output, XtoC_output, pooled_output = self.forward_linear(inputs.input_ids, inputs.attention_mask, inputs.pixel_values)
    ...
    def evaluate_model_with_residual(self, data_loader, dataset_name =".."):
        ...
        with torch.no_grad():
            for batch in tqdm(data_loader, desc= dataset_name, unit="batch"):
                ...
                final_output, XtoC_output, _ = self.forward_linear(inputs.input_ids, inputs.attention_mask, inputs.pixel_values)
